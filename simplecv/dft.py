...
class DFT(object):
    ...
    **EXAMPLE**
    ...
    def __init__(self, **kwargs):
        ...
        self._numpy = kwargs.get('numpyarray')
        self._image = kwargs.get('image')
        self._dia = kwargs.get('dia', 0)
        self._type = kwargs.get('type', '')
        self._order = kwargs.get('order', 0)
        self._freqpass = kwargs.get('frequency', '')
        self._x_cutoff_low = kwargs.get('x_cutoff_low', 0)
        self._y_cutoff_low = kwargs.get('y_cutoff_low', 0)
        self._x_cutoff_high = kwargs.get('x_cutoff_high', 0)
    ...
    def __add__(self, flt):
        if not isinstance(flt, type(self)):
            logger.warn("Provide SimpleCV.DFT object")
            return None
        ...
        **EXAMPLE**
        ...
        if isinstance(dia, list):
            if len(dia) != 3 and len(dia) != 1:
                logger.warn("diameter list must be of size 1 or 3")
                return None
            stacked_filter = DFT()
            for d in dia:
                stacked_filter = stacked_filter._stack_filters(
                    self.create_gaussian_filter(d, size, highpass))
            image = Factory.Image(stacked_filter._numpy)
    ...
    def create_butterworth_filter(self, dia=400, size=(64, 64), order=2,
        ...
        **EXAMPLE**
        ...
        ...                                     highpass=True)
        >>> flt2 = DFT.create_butterworth_filter([100, 120, 140], (512, 512),
        ...
        if isinstance(dia, list):
            ...
            image = Factory.Image(stackedfilter._numpy)
        ...
        **EXAMPLE**
        ...
        if isinstance(x_cutoff, list):
            if len(x_cutoff) != 3 and len(x_cutoff) != 1:
                logger.warn("x_cutoff list must be of size 3 or 1")
                return None
            if isinstance(y_cutoff, list):
                if len(y_cutoff) != 3 and len(y_cutoff) != 1:
                    logger.warn("y_cutoff list must be of size 3 or 1")
                    return None
                if len(y_cutoff) == 1:
    ...
    def create_bandpass_filter(self, x_cutoff_low, x_cutoff_high,
        ...
        **EXAMPLE**
        ...
        img = Factory.Image(bandpassnumpy)
    ...
    def create_notch_filter(self, dia1, dia2=None, cen=None, size=(64, 64),
        ...
        **EXAMPLE**
        ...
        >>> notch2 = DFT.create_notch_filter(dia1=200, dia2=300,
        ...                                  cen=(200, 200),
        ...                                  size=(512, 512))
        >>> img = Image('lenna')
        ...
        if isinstance(dia1, list):
            ...
            if isinstance(dia2, list):
                if len(dia2) != 3 and len(dia2) != 1:
                    logger.warn("diameter list must be of size 3 or 1")
                    return None
                if len(dia2) == 1:
            ...
            if isinstance(cen, list):
                if len(cen) != 3 and len(cen) != 1:
                    logger.warn("center list must be of size 3 or 1")
                    return None
                if len(cen) == 1:
                    cen = [cen[0]] * len(dia1)
            else:
    ...
    def stack_filters(self, flt1, flt2):
        ...
        **EXAMPLE**
        ...
        if not (self.channels == 1
                and flt1.channels == 1
                and flt2.channels == 1):
            logger.warn("Filters must have only 1 channel")
            return None
        if not (self.size() == flt1.size() and self.size() == flt2.size()):
            logger.warn("All the filters must be of same size")
            return None
