...
&nbsp;
            ...
            lasttimeserved = 0
            while 1:
                if _jpegstreamers[port].refreshtime > lasttimeserved \
                        or time.time() - timeout > lasttimeserved:
                    try:
                        self.wfile.write("--BOUNDARYSTRING\r\n")
                        self.send_header("Content-type", "image/jpeg")
                        self.send_header(
                            "Content-Length",
                            str(len(_jpegstreamers[port].jpgdata.getvalue())))
                        self.end_headers()
                        self.wfile.write(
...
class JpegStreamer(object):
    ...
    - sleep time (default 0.1) how often to update.  Above 1 second seems to
     cause dropped connections in Google chrome
    ...
    def __init__(self, hostandport=8080, st=0.1):
        ...
        # self referential, ugh.  but gives some bkcompat
        self.framebuffer = self

    def url(self):
        """
        Returns the JpegStreams Webbrowser-appropriate URL, if not provided in
        the constructor, it defaults to "http://localhost:8080"
        """
...
class VideoStream(object):
    ...
    You can also specify a framerate, and if you want to "fill" in missed
    frames.
    ...
    Where if you want to do a stop-motion animation, you would want to turn
    fill off::

        vs_animation = VideoStream("cartoon.avi", 15, False)


    If you select a fill, the VideoStream will do its best to stay close to
    "real time" by duplicating frames or dropping frames when the clock doesn't
    sync up with the file writes.
