...
def lkTracker(img, bb, ts, oldimg, **kwargs):
    ...
    * *oldimg* - Image - Previous Image.
    ...
    qualityLevel  - Parameter characterizing the minimal accepted quality of
                    ...
                    rejected. For example, if the best corner has the quality
                    measure = 1500, and the qualityLevel=0.01 , then all the
    ...
    maxLevel      - 0-based maximal pyramid level number; if set to 0, pyramids
                    are not used (single level), if set to 1, two levels are
                    used, and so on
    ...
    **HOW TO USE**
    ...
    >>> while some_condition_here:
        ...
        ... bb = ts[-1].bb
        ... prevImg = img
        ... ts = lkTracker(img, bb, ts, prevImg, maxCorners=4000,
        ...                qualityLevel=0.5, winSize=(12,12))
        ... ts[-1].drawBB()
        ... img.show()
    ...
    It is based on LK Optical Flow. It calculates Optical flow in frame1 to
    frame2 and also in frame2 to frame1 and using back track error, filters out
    false positives.
    """
    maxCorners = 4000
    qualityLevel = 0.08
