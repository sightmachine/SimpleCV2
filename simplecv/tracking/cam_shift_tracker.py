...
def camshiftTracker(img, bb, ts, **kwargs):
    ...
    **HOW TO USE**
    ...
    with single tone or tracked very efficiently. CAMshift should be preferred
    if you are trying to track faces. It is optimized to track faces.
    """
    ...
    hsv = cv2.cvtColor(img.get_numpy_cv2(), cv2.cv.CV_BGR2HSV)
    if mask is None:
