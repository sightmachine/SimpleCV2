...
def mfTracker(img, bb, ts, oldimg, **kwargs):
    ...
    **HOW TO USE**
    ...
    median of the flow, and eliminates
    points.
    """
    numM = 10
    numN = 10
    ...
    oldg = oldimg.gray_ndarray
    newg = img.gray_ndarray
...
def lktrack(img1, img2, ptsI, nPtsI, winsize_ncc=10, win_size_lk=4,
    ...
    **PARAMETERS**
    ...
    img1 - Previous image or image containing the known bounding box (Numpy
     array)
    img2 - Current image
    ...
    winsize_ncc - size of the search window at each pyramid level in LK tracker
     (in int)
    method - Paramete specifying the comparison method for normalized cross
    correlation (see http://opencv.itseez.com/modules/imgproc/doc/
    object_detection.html?highlight=matchtemplate#cv2.matchTemplate)

    **RETURNS**

    fb - forward-backward confidence value. (corresponds to euclidean distance
     between).
...
def getFilledBBPoints(bb, numM, numN, margin):
    ...
    **RETURNS**
    ...
    """
    pointDim = 2
    bb_local = (bb[0] + margin, bb[1] +
                margin, bb[2] - margin, bb[3] - margin)
    if numM == 1 and numN == 1:
        ...
        for i in range(numN):
            for j in range(numM):
                ...
                pt[i * numM * pointDim + j * pointDim + 1] = \
                    bb_local[1] + j * spaceM
        ...
        for i in range(numN):
            for j in range(numN):
                pt[i * numM * pointDim + j * pointDim + 0] = \
                    bb_local[0] + i * spaceN
    ...
    for i in range(numN):
        for j in range(numM):
            pt[i * numM * pointDim + j * pointDim + 0] = float(
                bb_local[0] + i * spaceN)
            pt[i * numM * pointDim + j * pointDim + 1] = float(
                bb_local[1] + j * spaceM)
    return pt
    ...
    **RETURNS**
    ...
    for i in range(nPts):
        for j in range(i + 1, nPts):
            temp0 = ((pt0[i][0] - pt0[j][0]) ** 2 +
                     (pt0[i][1] - pt0[j][1]) ** 2) ** 0.5
            temp1 = ((pt1[i][0] - pt1[j][0]) ** 2 +
                     (pt1[i][1] - pt1[j][1]) ** 2) ** 0.5
            if temp0 != 0:
...
def normCrossCorrelation(img1, img2, pt0, pt1, status, winsize,
                         method=cv2.cv.CV_TM_CCOEFF_NORMED):
    """
    **SUMMARY**
    (Dev Zone)
    ...
    **PARAMETERS**
    ...
    winsize- Size of quadratic area around the point
             which is compared.
    method - Specifies the way how image regions are compared.
     see cv2.matchTemplate
