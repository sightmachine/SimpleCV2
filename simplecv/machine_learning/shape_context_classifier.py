...
class ShapeContextClassifier():
    ...
    def _getMatch(self, model_scd, test_scd):
        ...
        return self._matchQuality(distance)
    ...
    def _doMatching(self, model_name, test_scd):
        ...
        #metric = 1.0 + np.log10( np.max([myPts,otPts])/np.min([myPts,otPts]))
        otherIdx = []
        distance = []
        import warnings
    ...
    def _buildMatchDict(self, image, countBlobs):
        ...
        for key, value in self.descMap.items():
            ...
            if countBlobs and self.blobCount[key] == count:
    ...
    def classify(self, image, blobFilter=True):
        ...
        * *blobFilter* - Do a first pass where you only match objects
          ...
          and match quality.
        """
        points, descriptors, count, matchDict, matchStd = self._buildMatchDict(
            image, blobFilter)
        best = sys.maxint
        best_name = "No Match"
