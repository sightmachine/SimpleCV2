...
class TemporalColorTracker:
    ...
    **SUMMARY**
    ...
    def train(self, src, roi=None, extractor=None, doCorr=False,
              maxFrames=1000,
              ssWndw=0.05, pkWndw=30, pkDelta=3, corrStdMult=2.0,
        ...
        * *extractor* - A function with the following signature:
          (R,G,B) = Extract(Image)
        * *doCorr* - Do correlation use correlation to confirm that the signal
         is present.
        ...
        * *pkDelta* - The minimum difference between the steady state to look
         for peaks.
        * *corrStdMult* - The maximum correlation standard deviation of the
    ...
    def _findSteadyState(self, windowSzPrct=0.05):
        ...
        for key in self.data.keys():
            ...
            data = [np.std(signal[i:i + wndwSz]) for i in
                    range(0, len(signal) - wndwSz)]
                    ...
                    if l[0] == wndwCenter and l[1] > self._cutoff:
                        if self.doCorr:
                            corrVal = np.correlate(self._rtData.normalize(),
                                                   self._template)
                            thresh = self.corrThresh[0] - self.corrStdMult * \
                                self.corrThresh[1]
                            if corrVal[0] > thresh:
                                self.count += 1
                        else:
