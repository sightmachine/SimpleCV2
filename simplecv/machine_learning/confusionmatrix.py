...
class ConfusionMatrix():
    ...
    def addDataPoint(self, truth_name, test_name):
        self.confusionMatrix[self.nameMap[truth_name]][
            self.nameMap[test_name]] += 1
    ...
    def getCorrectPercent(self):
        if self.totalCount > 0 and self.correctCount:
            return np.around(float(self.correctCount) / float(self.totalCount),
                             4)
        else:
            return 0.00
    ...
    def getIncorrectPercent(self):
        if self.totalCount > 0 and self.correctCount:
            return np.around(
                float(self.incorrectCount) / float(self.totalCount), 4)
        else:
            return 0.00

    def getClassCorrectPercent(self, className):
        total = float(np.sum(self.confusionMatrix[:, self.nameMap[className]]))
        correct = float(self.confusionMatrix[
            self.nameMap[className], self.nameMap[className]])
    ...
    def toString(self, pad_sz=7):
        ...
        retVal += "Incorrect " + str(
            100.00 * self.getIncorrectPercent()) + "% \n"
        retVal += 50 * '#'
        retVal += '\n'
        wrdLen = 0
        sz = pad_sz
