...
def test_image_drawing():
    ...
    img.drawCircle((img.width / 2, img.height / 2), 15, thickness=5,
                   color=Color.RED)
    img.drawCircle((img.width / 2, img.height / 2), 20)
    img.drawLine((5, 5), (5, 8))
    img.drawLine((5, 5), (10, 10), thickness=3)
    img.drawLine((0, 0), (img.width, img.height), thickness=3,
                 color=Color.BLUE)
...
def test_detection_lines():
    ...
    if lines is 0 or lines is None:
        assert False
...
def test_applySideBySide():
    ...
    # BL = big image little image  -> this is important
    # to test all the possible cases.
    results = []
...
def test_keypoint_match():
    ...
    fs1 = match1.findKeypointMatch(template, quality=300.00, minDist=0.5,
                                   minMatch=0.2)
    fs3 = match3.findKeypointMatch(template, quality=300.00, minDist=0.5,
                                   minMatch=0.2)
    print "This should fail"
    fs2 = match2.findKeypointMatch(template, quality=500.00, minDist=0.2,
                                   minMatch=0.1)
...
def test_draw_keypoint_matches():
    ...
    template = Image("../data/sampleimages/KeypointTemplate2.png")
    match0 = Image("../data/sampleimages/kptest0.png")
    result = match0.drawKeypointMatches(template, thresh=500.00, minDist=0.15,
                                        width=1)
...
def test_biblical_flood_fill():
    ...
    img.floodFill(b.coordinates(), tolerance=(3, 3, 3), color=Color.GREEN,
                  fixed_range=False)
...
def test_flood_fill_to_mask():
    ...
    omask2 = img.floodFillToMask(b.coordinates(), tolerance=(3, 3, 3),
                                 mask=imask)
    omask3 = img.floodFillToMask(b.coordinates(), tolerance=(3, 3, 3),
...
def test_bandPassFilter():
    ...
    d = img.bandPassFilter(0.1, 0.3, yCutoffLow=0.1, yCutoffHigh=0.3,
                           grayscale=True)
    e = img.bandPassFilter([0.1, 0.2, 0.3], [0.5, 0.5, 0.5])
    f = img.bandPassFilter([0.1, 0.2, 0.3], [0.5, 0.5, 0.5],
...
def test_image_new_smooth():
    ...
    result.append(
        img.bilateralFilter(diameter=14, sigmaColor=20, sigmaSpace=34))
    ...
    result.append(
        img.gaussianBlur((7, 9), sigmaX=10, sigmaY=12, grayscale=True))
