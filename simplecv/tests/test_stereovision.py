...
from simplecv.camera import StereoImage, StereoCamera, Camera
...
def test_loadCalibration():
    cam = StereoCamera()
    calbib = cam.loadCalibration("Stereo", "../data/test/StereoVision/")
    if (calbib):
        assert True
    else:
        assert False
...
def test_StereoRectify():
    cam = StereoCamera()
    calib = cam.loadCalibration("Stereo", "../data/test/StereoVision/")
